...
class ArmEnv(object):
    ...
    def step(self,action):
        ...
        r=-np.sqrt(state[1]**2)*math.exp(p/10)
        print({"rewards"},r)
        if p<0.4:
            self.on_goal +=1
            if self.on_goal >=1:
    ...
    def reset(self):
        self.goal[0] = np.random.uniform(145,165)
        #self.goal[1] = np.random.uniform(135,135)
        self.goal[2] = np.random.uniform(1179,1188)
        self.goal[1] = np.random.uniform(135,145)
        self.goal[1] =128.23502
        #self.goal[2] = 1182.856445
        self.on_goal = 0
        Coor = get_obs()
        obs = [Coor[1]]
        #dist= [(self.goal[0]-obs_[0]),(self.goal[1]-obs[1]),(self.goal[2]-obs[2])]
        obs = [(self.goal[1]-obs[0])]
        return obs
